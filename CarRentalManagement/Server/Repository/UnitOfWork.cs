...
namespace CarRentalManagement.Server.Repository
{
    public class UnitOfWork : IUnitOfWork
    {
        ...
        public async Task Save(HttpContext httpContext)
        {
            ...
            {
                ((BasedDomainModel)entry.Entity).DateUpdated = DateTime.Now;
                ((BasedDomainModel)entry.Entity).UpdatedBy = user;
                if (entry.State == EntityState.Added)
                {
                    ((BasedDomainModel)entry.Entity).DateCreated = DateTime.Now;
                    ((BasedDomainModel)entry.Entity).CreatedBy = user;
                }
            }
